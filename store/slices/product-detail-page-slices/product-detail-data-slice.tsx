...
export const ProductDetailPageThunk = createAsyncThunk('product-detail-data-slice/fetchProductDetailData', async (params: any) => {
...
const productDetailSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(ProductDetailPageThunk.fulfilled, (state, action) => {
      if (action.payload.status === 200 && action.payload.data.hasOwnProperty('message') && action.payload.data.message.msg !== 'error') {
...
export const product_detail_data_selector_state = (state: RootState) => state.ProductDetailDataScreen;
