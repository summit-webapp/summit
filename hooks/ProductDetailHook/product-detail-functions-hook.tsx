...
const useProductDetailFunctions = () => {
  ...
  const [generateThumbnailOfVariants, setGenerateThumbnailOfVariants] = useState<any>({});

  const handleSettingOfSelectedVariantsAndThumbnailOfVariants = (variantsDataFromAPI: any) => {
    ...
    variantsDataFromAPI?.attributes?.forEach((attribute: any) => {
      temporaryStoreForSelectedVariant[attribute.field_name] = attribute.default_value;
    });

    console.log('working for modal slug from api selected variant', temporaryStoreForSelectedVariant);
    setGenerateSelectedVariants({ ...temporaryStoreForSelectedVariant });

    const displayThumbnailAttributes: any = variantsDataFromAPI?.attributes?.filter((attr: any) => attr.display_thumbnail === true);

    console.log('working for modal slug from api display thumbnail', displayThumbnailAttributes);

    var imgsValObj: any = displayThumbnailAttributes?.reduce((acc: any, curr: any) => {
      acc[curr.field_name] = [];
      return acc;
    }, {});
    ...
    displayThumbnailAttributes?.forEach((attr: any) => {
      attr?.values?.map((value: any) => {
        variantsDataFromAPI?.variants?.find((variant: any) => {
          if (variant[attr.field_name] === value && variant.image.length !== 0) {
    ...
    console.log('working for modal slug check generated values', generateSelectedVariants, generateThumbnailOfVariants);
